...
@Component({
  ...
  styleUrls: ['./estado-res.component.css']
})
export class EstadoResComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.tipo=="2"){
      this.url="http://localhost:5002/restaurante/pedido/"+this.id_pedido;
      this.service.get_estado_pedido(parseInt(this.id_pedido,10)).subscribe((data:respuesta) => {
        //alert(data);
        console.log(data);
        this.actualizar_tracker(data);
      });
    }else{
      this.url="http://localhost:5003/restaurante/pedido/"+this.id_pedido;
      this.service.get_estado_pedido_orq(parseInt(this.id_pedido,10)).subscribe((data:respuesta) => {
        //alert(data);
        console.log(data);
        this.actualizar_tracker(data);
      });
